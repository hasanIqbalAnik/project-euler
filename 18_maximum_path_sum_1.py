...
00 20 04 82 47 65 00 00 00 00 00 00 00 00 00 00 
00 19 01 23 75 03 34 00 00 00 00 00 00 00 00 00 
00 88 02 77 73 07 63 67 00 00 00 00 00 00 00 00 
00 99 65 04 28 06 16 70 92 00 00 00 00 00 00 00 
...
00 04 62 98 27 23 09 70 98 73 93 38 53 60 04 23"""
...
for line in lines:
	arrOfint.append([int(x) for x in line.strip().split(' ')])

arrOfint = arrOfint[1:len(arrOfint)]	
for i in xrange(1, 16, 1):
	for j in xrange(1, 16, 1):
		arrOfint[i][j] = max(arrOfint[i][j] + arrOfint[i-1][j], arrOfint[i][j] + arrOfint[i-1][j-1])
